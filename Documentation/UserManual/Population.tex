...
\begin{itemize}
    \item Total catch (catch-on-board), $C_{f}$, is calculated by VF/VR*(retained catch), where VF is vulnerable retained biomass, $j$ indexes categories and $t$ is the proportion of M in the time-step, and VF is the full vulnerable biomass, $VF = \sum_{a,j} \overline{w}_{a} S_{a,j} n_{a,j} \exp(-0.5 t M_{a,j})$.
    ...
    \item Discard numbers-at-age (including their share of natural mortality) is $S_{a,j} (1 - R_{a,j}) D_{a,j} n_{a,j} \exp(-0.5 t M_{a,j})$, and those that die at the end of the time step (updating the partition) are $S_{a,j} (1 - R_{a,j}) D_{a,j} n_{a,j} \exp(-t M_{a,j})$, where $D_{a,j}$ is the fraction that die on return to the sea.

    \item The partition is updated by removing landed catch, natural mortality, and discard mortality
        $$ n'_{a} = n_{a} \exp(-t M_a) \big[ 1 - \sum_f S_{f,a} U_f (R_{f,a} + (1 - R_{f,a}) D_{f,a}) \big] $$
\end{itemize}
