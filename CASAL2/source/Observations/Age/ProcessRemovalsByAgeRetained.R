...
# Now create observation for age retained
sRet <- sell(age, 7, 0.9) # selection for a50 = 7, ato95 = 0.9
obs_ret <- sRet$y * Ncaught # retained catch
obs_ret = obs_ret[3:15]/sum(obs_ret[3:15]) # normalised
exp_ret = Nretained[3:15]/sum(Nretained[3:15]) # numbers from guess parameters in input file
age_retained = cbind(age[3:15], obs_ret, exp_ret) # use these values in unit test
colnames(age_retained) = c('age', 'observed', 'expected')
