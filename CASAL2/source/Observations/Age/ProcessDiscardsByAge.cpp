...
// Headers
#include "ProcessDiscardsByAge.h"
 ...
 * Default constructor
 */
ProcessDiscardsByAge::ProcessDiscardsByAge(Model* model) : Observation(model) {
 ...
 * Destructor
 */
ProcessDiscardsByAge::~ProcessDiscardsByAge() {
  delete obs_table_;
 ...
 */
void ProcessDiscardsByAge::DoValidate() {
 ...
 */
void ProcessDiscardsByAge::DoBuild() {
 ...
 */
void ProcessDiscardsByAge::PreExecute() {
}

/**
 *
 */
void ProcessDiscardsByAge::Execute() {
  LOG_TRACE();
  ...
  unsigned current_time_step = model_->managers().time_step()->current_time_step();
	if (time_step_to_execute_ == current_time_step) {
		...
		map<unsigned,map<string, map<string, vector<Double>>>> &discards_at_age = mortality_instantaneous_->discard_data();
			...
			for (; category_iter != partition_iter->end(); ++category_iter) {
				...
				for (string fishery : method_) {
				  // This should get caught in the DoBuild now.
					if (discards_at_age[year][fishery][(*category_iter)->name_].size() == 0) {
					...
					if (ageing_error_label_ != "") {
						...
						vector<Double> temp(discards_at_age[year][fishery][(*category_iter)->name_].size(), 0.0);
						LOG_FINEST() << "category = " << (*category_iter)->name_;
						LOG_FINEST() << "size = " << discards_at_age[year][fishery][(*category_iter)->name_].size();

						for (unsigned i = 0; i < mis_matrix.size(); ++i) {
							for (unsigned j = 0; j < mis_matrix[i].size(); ++j) {
								temp[j] += discards_at_age[year][fishery][(*category_iter)->name_][i] * mis_matrix[i][j];
							}
						}
						discards_at_age[year][fishery][(*category_iter)->name_] = temp;
					}
					LOG_TRACE();
					/*
					 *  Now collapse the number_age into the expected_values for the observation
					 */
					for (unsigned k = 0; k < discards_at_age[year][fishery][(*category_iter)->name_].size(); ++k) {
						LOG_FINE() << "----------";
						LOG_FINE() << "Fishery: " << fishery;
						LOG_FINE() << "Numbers At Age After Ageing error: " << (*category_iter)->min_age_ + k << "for category " << (*category_iter)->name_ << " " << discards_at_age[year][fishery][(*category_iter)->name_][k];

						unsigned age_offset = min_age_ - model_->min_age();
						if (k >= age_offset && (k - age_offset + min_age_) <= max_age_)
						expected_values[k - age_offset] = discards_at_age[year][fishery][(*category_iter)->name_][k];
						// Deal with the plus group
						if (((k - age_offset + min_age_) > max_age_) && plus_group_)
						expected_values[age_spread_ - 1] += discards_at_age[year][fishery][(*category_iter)->name_][k];
					}
 ...
 */
void ProcessDiscardsByAge::CalculateScore() {
  /**
