...
# unit test: Process Removals By Length Retained Total
...
# Now create observation for length total
sTot <- sell(age, 5, 0.8) # selection for a50 = 5, ato95 = 0.8
obs_catch <- l_from_a(age, length_bins, sTot$y * N)  # all catch as length
obs_total = obs_catch/sum(obs_catch) # normalised
exp_total = l_from_a(age, length_bins, Ncaught) # numbers from guess parameters in input file
exp_total = exp_total/sum(exp_total) # scale to 1
length_total = cbind(length_bins[1:(length(length_bins)-1)], obs_total, exp_total) # use these values in unit test
colnames(length_total) = c('length', 'observed', 'expected')
